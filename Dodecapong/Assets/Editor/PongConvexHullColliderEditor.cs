...
public class PongConvexHullColliderEditor : PongColliderEditor
{
    private void OnSceneGUI()
    {
        ...
        for (int i = 0; i < points.Length; i++) {
            points[i] = (Vector3)collider.points[i] + collider.transform.position;
        }
        ...
        for (int i = 0; i < points.Length; i++) {
            collider.points[i] = Handles.FreeMoveHandle(points[i], Quaternion.identity, 0.05f, new Vector3(0.1f, 0.1f, 0.0f), Handles.DotHandleCap) - collider.transform.position;
        }
        ...
        for (int i = 0; i < points.Length; i++) {
            ...
            Handles.DrawLine(midPoint, midPoint + (Vector3)collider.normals[i]);
        }
    }
